        ...
        {result && (
          <div className="mt-10">
            ...
            <div className="rounded-2xl border border-border/60 bg-card p-5 mb-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${tier === 'advanced' ? 'bg-purple-500/10 border border-purple-500/20' : 'bg-teal-500/10 border border-teal-500/20'}`}>
                    {tier === 'advanced' ? <Crown className="w-4 h-4 text-purple-400" /> : <Mic2 className="w-4 h-4 text-teal-500" />}
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-foreground">{file?.name || 'Audio Feedback'}</p>
                    <p className={`text-xs font-medium ${tier === 'advanced' ? 'text-purple-400/70' : 'text-teal-500/70'}`}>
                      {tier === 'advanced' ? 'Advanced Analysis · Gemini 2.5 Pro' : 'Base Analysis · Gemini 2.5 Flash'}
                    </p>
                  </div>
                </div>
                <div className="flex gap-2">
                  <button onClick={reset}
                    className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium text-muted-foreground border border-border/60 hover:text-foreground hover:border-border transition-all">
            ...
            <div className={`rounded-2xl border p-8 bg-gradient-to-b to-transparent ${
              tier === 'advanced' ? 'border-purple-500/20 from-purple-500/5' : 'border-teal-500/20 from-teal-500/5'
            }`}>
              <ReactMarkdown
                className={`prose prose-invert max-w-none
                  ...
                  ${ tier === 'advanced' ? 'prose-headings:text-purple-300' : 'prose-headings:text-teal-300' }`}
                components={{
                  h2: ({ children }) => (
                    <div className="mt-10 mb-4">
                      <div className="border-t border-border/40 mb-6" />
                      <h2 className={`text-2xl font-bold ${tier === 'advanced' ? 'text-purple-300' : 'text-teal-300'}`}>{children}</h2>
                    </div>
                  ),
                }}
              >
